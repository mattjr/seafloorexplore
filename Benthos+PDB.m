...
- (void)createBondsForPDBResidue:(NSString *)residueType withAtomDictionary:(NSDictionary *)atomDictionary structureNumber:(NSInteger)structureNumber;
{
	...
	BenthosResidueType residueIdentifier = [[pdbResidueLookupTable objectForKey:residueType] intValue];
	...
	while (lineEnd < length) 
	{
		...
		if ([currentLine length] >= 6) // Make sure that we at least have a line identifier present, move on the the next line otherwise
		{
			...
			if (([lineIdentifier isEqualToString:@"ATOM"]) || ([lineIdentifier isEqualToString:@"HETATM"]))
			{
				...
				[atomCoordinates setObject:[NSValue valueWithBytes:&atomCoordinate objCType:@encode(Benthos3DPoint)] forKey:[NSNumber numberWithInt:atomSerialNumber]];
				if (![lineIdentifier isEqualToString:@"HETATM"])
				{
					...
					[residueAtoms setObject:[NSValue valueWithBytes:&atomCoordinate objCType:@encode(Benthos3DPoint)] forKey:atomResidueIdentifier];
				}
				...
				BenthosAtomType processedAtomType;
