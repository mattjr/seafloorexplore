...
*/
#define fullpath @"/Users/mattjr/Downloads/st_helens_11"
...
- (void)prepareOpenGL
{
    ...
    id sim = [[[Simulation alloc] initWithString:fullpath withScene:scene] autorelease];
	if (sim)
...
- (void)mouseDown:(NSEvent *)theEvent
{
    ...
    [[scene simulator] setValidPos];

	if ([[scene simulator] respondsToSelector:@selector(mouseDown:)])		[[scene simulator] pancont:pt];
...
- (void)mouseUp:(NSEvent *)theEvent
{
	...
	if ([[scene simulator] respondsToSelector:@selector(mouseUp:)])			[[scene simulator] pancont:pt];
}
...
- (void)mouseDragged:(NSEvent *)theEvent
{
	if ([[scene simulator] respondsToSelector:@selector(mouseDragged:withFlags:)]){
		...
		[(Simulation *)[scene simulator] mouseDragged:pt withFlags:(uint32_t)[theEvent modifierFlags]];
    }
}
